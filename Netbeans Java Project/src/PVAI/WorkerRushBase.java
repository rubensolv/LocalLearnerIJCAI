...
public class WorkerRushBase extends AbstractionLayerAI {
    Random r = new Random();
    ...
    public WorkerRushBase(UnitTypeTable a_utt) {
        this(a_utt, new AStarPathFinding());
    }

        
    public WorkerRushBase(UnitTypeTable a_utt, PathFinding a_pf) {
        super(a_pf);
        reset(a_utt);
    }
    ...
    public AI clone() {
        return new WorkerRushBase(utt, pf);
    }
    
    @Override
    public AI clone_for_Thread() {
        return new WorkerRushBase(utt, new AStarPathFinding());
    }
...
//        System.out.println("LightRushAI for player " + player + " (cycle " + gs.getTime() + ")");
    ...
    public void meleeUnitBehavior(Unit u, Player p, GameState gs) {
        ...
        for(Unit u2:pgs.getUnits()) {
            if (u2.getPlayer()>=0 && u2.getPlayer()!=p.getID() && (u2.getType() == baseType || u2.getType() == barracks)) { 
            ...
            {
                int d = Math.abs(u2.getX() - u.getX()) + Math.abs(u2.getY() - u.getY());
                if (closestMeleeEnemy==null || d<enemyDistance) {
                    closestMeleeEnemy = u2;
                    enemyDistance = d;
                }
            }
        }
        if (closestEnemy!=null && enemyDistance > 3) {
            attack(u,closestEnemy);
        }
        else if(closestMeleeEnemy != null)
        {
            attack(u,closestMeleeEnemy);
        }
    }
