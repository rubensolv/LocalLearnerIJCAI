 ...
 * @author rubensolv
 */
public class EconomyLightRush extends AbstractionLayerAI {
    ...
    public EconomyLightRush(UnitTypeTable a_utt) {
        this(a_utt, new AStarPathFinding());
    }

    public EconomyLightRush(UnitTypeTable a_utt, PathFinding a_pf) {
        super(a_pf);
        reset(a_utt);
    }
    ...
    public PlayerAction getAction(int player, GameState gs) throws Exception {
        ...
        if (pgs.getHeight() <= 16 && pgs.getHeight() <= 16)
            nWorkerBase = 3;
        // behavior of bases:
    ...
    public AI clone_for_Thread() {
         return new EconomyLightRush(utt, new AStarPathFinding());
    }
    ...
    public void baseBehavior(Unit u, Player p, PhysicalGameState pgs) {
        ...
        if (p.getResources() > (workerType.cost + resourcesUsed) + 5) {
            train(u, workerType);
            resourcesUsed += workerType.cost;
        }
    }
...
//        }
    ...
    public void workersBehavior(List<Unit> workers, Player p, PhysicalGameState pgs) {
        ...
        ArrayList<Unit> hasResource = new ArrayList<>();
        while (freeWorkers.size() > nWorkerBase) {
            Unit u = freeWorkers.remove(0);
            if (u.getResources() > 0) {
                hasResource.add(u);
            }else {
        ...
        harvestWorkers(hasResource, p, pgs);
