...
/**
 *
 * @author santi and rubens
 * 
 * Esta versão utiliza o PlayerActionGenerator_Asymmetric para gerar os movimentos das unidades.
 */
public class IDABCDAsymmetric_ABActionGeneration extends AIWithComputationBudget implements InterruptibleAI {
    ...
    List<ABCDNode_Asymmetric> stack = null;
    ...
    public IDABCDAsymmetric_ABActionGeneration(UnitTypeTable utt) {
        this(100, -1,
                new WorkerRushPlusPlus(utt, new AStarPathFinding()), 100,
                new SimpleSqrtEvaluationFunction3(), true);
    }

    public IDABCDAsymmetric_ABActionGeneration(int tpc, int ppc, AI a_playoutAI, int a_maxPlayoutTime, EvaluationFunction a_ef, boolean a_performGreedyActionScan) {
        super(tpc, ppc);
    ...
    public AI clone() {
        return new IDABCDAsymmetric_ABActionGeneration(TIME_BUDGET, ITERATIONS_BUDGET, playoutAI, maxPlayoutTime, ef, performGreedyActionScan);
    }
...
//            if (depth==50) DEBUG = 2;
            ...
            if (DEBUG >= 1) {
                System.out.println("    Time taken: " + (System.currentTimeMillis() - currentTime) + ", nPlayouts: " + nPlayouts);
            }
...
//        return bestMove;
    ...
    public PlayerAction getBestActionSoFar() throws Exception {
        ...
        if (bestMove == null) {
            PlayerActionGenerator_Asymmetric pag = new PlayerActionGenerator_Asymmetric(gs_to_start_from, playerForThisComputation, currentScriptData, _unitsAbsAB);
            PlayerAction pa = pag.getRandom();
            //pa = defineAssymetricAction(gs_to_start_from, nLeaves, pa);
            return pa;
        }
        //System.out.println("IDABCDAsymmetric Maior profundidade = "+this.statisticsString());
        return bestMove;
    }
    ...
    public PlayerAction greedyActionScan(GameState gs, int player, long cutOffTime, int maxPlayouts) throws Exception {
        ...
        float bestScore = 0;
        PlayerActionGenerator_Asymmetric pag = new PlayerActionGenerator_Asymmetric(gs, player, currentScriptData, _unitsAbsAB);
...
//        System.out.println(pag);
        do {
            ...
            if (pa != null) {
                //pa = defineAssymetricAction(gs, player, pa);
    ...
    public PlayerAction searchOutsideStack(GameState initial_gs, int maxplayer, int minplayer, int depth, long cutOffTime, int maxPlayouts, boolean needAResult) throws Exception {
        ABCDNode_Asymmetric head;
        if (stack == null) {
//            System.out.println("searchOutsideStack: stack is null (maxplayer: " + maxplayer + ")");
            nLeaves = 0;
            time_depth = 0;
            stack = new LinkedList<ABCDNode_Asymmetric>();
            head = new ABCDNode_Asymmetric(-1, 0, initial_gs, -EvaluationFunction.VICTORY, EvaluationFunction.VICTORY, 0);
            stack.add(head);
            treeIsComplete = true;
        } else {
...
//            System.out.println("]");
            ABCDNode_Asymmetric current = stack.get(0);
...
//                                System.out.println("last result from -1 node");
                ...
                case 0: // max node:
                    ...
                    if (current.actions == null) {
                        current.actions = new PlayerActionGenerator_Asymmetric(current.gs, maxplayer, currentScriptData, _unitsAbsAB);
                        ...
                        if (next != null) {
                            ...
                            stack.add(0, new ABCDNode_Asymmetric(-1, current.depth + 1, gs2, current.alpha, current.beta, current.nextPlayerInSimultaneousNode));
                        } else {
                ...
                case 1: // min node:
                    nNodes++;
                    if (current.actions == null) {
                        current.actions = new PlayerActionGenerator_Asymmetric(current.gs, minplayer,currentScriptData, _unitsAbsAB);
                        current.actions.randomizeOrder();
                        long l = current.actions.getSize();
                        if (DEBUG >= 2) {
                            for (int i = 0; i < current.depth; i++) {
                        ...
                        if (current.best == null || lastResult.m_b < current.best.m_b) {
                            ...
                            //current.best.m_a = current.actions.getLastAction();
                            playoutAI.reset();
                            current.best.m_a = playoutAI.getAction(minplayer, current.gs);
                        }
                        //PlayerAction next = current.actions.getNextAction(cutOffTime);
                        playoutAI.reset();
                        PlayerAction next = playoutAI.getAction(minplayer, current.gs);
...
//        System.out.println("searchOutsideStack: stack is not empty.");
        if (needAResult) {
            if (head.best != null) {
                _bestScore = (double) head.best.m_b;
                return head.best.m_a;
                //return defineAssymetricAction(head.gs, playerForThisComputation, head.best.m_a);
            }
            return head.actions.getRandom();
            //return defineAssymetricAction(head.gs, playerForThisComputation, head.actions.getRandom());
        }
        return null;
    }

    public String statisticsString() {
        return "avg depth: " + (avg_depth_so_far / (double) count_depth_so_far)
    ...
    public void setMaxPlayoutTime(int maxPlayoutTime) {
        this.maxPlayoutTime = maxPlayoutTime;
    }
    ...
    private PlayerAction defineAssymetricAction(GameState gs, int player, PlayerAction pa) throws Exception {
        //farei uma mesclagem entre as ações sugeridas pelo AB e os scripts contidos no unitScript.
        PlayerAction paT = new PlayerAction();
        HashSet<Unit> units = getUnits(gs, player);
        //se o total de unidades controladas for igual retorno a ação do AB
        if (units.size() <= _unitsAbsAB.size()) {
            return pa;
        }

        for (Unit unit : units) {
            if (unitIsControlled(unit)) {
                //caso a unidade seja controlada irei injetar a ação normal
                UnitAction uAcTemp = pa.getAction(unit);
                if (uAcTemp != null) {
                    paT.addUnitAction(unit, uAcTemp);
                }
            } else {
                //caso a unidade não seja controlada, irei obter a ação do script
                AI ai = currentScriptData.getAIUnit(unit);
                ai.reset();
                PlayerAction acScript = ai.getAction(player, gs);
                UnitAction uAcTemp = acScript.getAction(unit);
                if (uAcTemp != null) {
                    paT.addUnitAction(unit, uAcTemp);
                }
    ...
    private UnitAction getUnitAction(Unit unit, PlayerAction pa) {
        for (Pair<Unit, UnitAction> tmp : pa.getActions()) {
            if (tmp.m_a.getID() == unit.getID()) {
                return tmp.m_b;
            }
        }
        return null;
    ...
    private boolean unitIsControlled(Unit un) {

        for (Unit u : _unitsAbsAB) {
            if (u.getID() == un.getID()) {
                return true;
            }
        }
    ...
    private HashSet<Unit> getUnits(GameState gs, int player) {
