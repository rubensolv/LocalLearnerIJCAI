...
implements InterruptibleAI {
    ...
    public SAS(UnitTypeTable utt) {
        this(100, 200, new SimpleSqrtEvaluationFunction3(), utt, new AStarPathFinding());
    }

    public SAS(UnitTypeTable utt, int numUnits, int numManager) {
        this(100, 200, new SimpleSqrtEvaluationFunction3(), utt, new AStarPathFinding(), numUnits, numManager);
    }

    public SAS(UnitTypeTable utt, int numUnits, int numManager, List<AI> IAsPort, String name) {
        this(100, 200, new SimpleSqrtEvaluationFunction3(), utt, new AStarPathFinding(), numUnits, numManager);
        this.name = name;
        this._sss.setNewPortfolio(IAsPort);
    }

    public SAS(UnitTypeTable utt, int max_playouts, int numUnits, int numManager, List<AI> IAsPort, String name) {
        this(100, max_playouts, new SimpleSqrtEvaluationFunction3(), utt, new AStarPathFinding(), numUnits, numManager);
        this.name = name;
        this._sss.setNewPortfolio(IAsPort);
    }

    public SAS(UnitTypeTable utt, int numUnits, int numManager, List<AI> IAsPort, List<AI> IAsABCD, String name) {
        this(100, 200, new SimpleSqrtEvaluationFunction3(), utt, new AStarPathFinding(), numUnits, numManager);
        this.name = name;
        this._sss.setNewPortfolio(IAsPort);
    }

    public SAS(int time, int max_playouts, EvaluationFunction e, UnitTypeTable a_utt, PathFinding a_pf) {
    ...
    public SAS(int time, int max_playouts, EvaluationFunction e, UnitTypeTable a_utt, PathFinding a_pf, int numUnits, int numManager) {
        super(time, max_playouts);
        this.evaluation = e;
        this.utt = a_utt;
        this.pf = a_pf;
    ...
    @Override
    public AI clone() {
        return new SAS(this._time, this._max_playouts, this.evaluation, this.utt, this.pf);
    }
    ...
    public double eval_mixed(int player, GameState gs, PlayerAction paPGS, AI aiEnemy) throws Exception {
        ...
        for (Pair<Unit, UnitAction> pair : temp) {
            newPaPGS.addUnitAction(gs2.getUnit(((Unit)pair.m_a).getID()), (UnitAction)pair.m_b);
        }
    ...
    public PlayerAction getBestActionSoFar() throws Exception {
        ...
        double bestScore = this._sss.eval(this.playerForThisComputation, this.gs_to_start_from, this.currentScriptData, this._sss.getEnemyScript());
        ...
        for (Unit unit : this._unitsAbsAB) {
            unit = this.gs_to_start_from.getUnit(unit.getID());
            while (System.currentTimeMillis() < start + (long)this.TIME_BUDGET) {
                boolean hasImproved = false;
                if (System.currentTimeMillis() >= start + (long)(this.TIME_BUDGET - 10)) {
                    return paSSS;
                }
                for (UnitAction uAct : unit.getUnitActions(this.gs_to_start_from)) {
                    if (!this.gs_to_start_from.isUnitActionAllowed(unit, uAct) || !unit.canExecuteAction(uAct, this.gs_to_start_from) || this.gs_to_start_from.getActionAssignment(unit) != null) continue;
                    PlayerAction paPGSTemp = paSSS.clone();
                    paPGSTemp.addUnitAction(unit, uAct);
                    double sum = 0.0;
                    for (int i = 0; i < 4; ++i) {
                        sum += this.eval_mixed(this.playerForThisComputation, this.gs_to_start_from, paSSS, this._sss.getEnemyScript());
                    }
                    double scoreTemp = sum / 4.0;
                    if (!(scoreTemp > bestScore)) continue;
                    paSSS = paPGSTemp.clone();
                    bestScore = scoreTemp;
                }
            }
        }
    ...
    public String toString() {
        return "SAS_" + this._numUnits + "_" + this._numManager;
    }
}