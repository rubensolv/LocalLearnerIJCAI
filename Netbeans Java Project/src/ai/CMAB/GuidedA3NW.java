 ...
 *
 */
public class GuidedA3NW extends AIWithComputationBudget implements InterruptibleAI {
    ...
    protected GuidedA3NWithinMCTSNode tree = null;
    ...
    public GuidedA3NW(UnitTypeTable utt) {
        this(100, -1, 100, 2,
                0.3f, 0.0f, 0.4f,
                new RandomBiasedAI(),
        ...
        this.scripts = new ArrayList(Arrays.asList(new AI[]{new Rojo(utt)}));
    }

    public GuidedA3NW(int available_time, int max_playouts, int lookahead, int max_depth,
    ...
    public GuidedA3NW(int available_time, int max_playouts, int lookahead, int max_depth, float e_l, float e_g, float e_0, AI policy, EvaluationFunction a_ef, boolean fensa) {
        super(available_time, max_playouts);
        MAXSIMULATIONTIME = lookahead;
        playoutPolicy = policy;
        MAX_TREE_DEPTH = max_depth;
        ...
        ef = a_ef;
        forceExplorationOfNonSampledActions = fensa;        
        this.behavior = "ManagerClosestEnemy";
        this.qtdUnits = 1;        
        this.name = "GA3N-Rojo";
    }

    public GuidedA3NW(int available_time, int max_playouts, int lookahead, int max_depth, float e_l, float e_g, float e_0, AI policy, EvaluationFunction a_ef, boolean fensa, String classGeneratorAction) {
        super(available_time, max_playouts);
        MAXSIMULATIONTIME = lookahead;
        playoutPolicy = policy;
        MAX_TREE_DEPTH = max_depth;
    ...
    public GuidedA3NW(int available_time, int max_playouts, int lookahead, int max_depth, float e_l, float e_g, float e_0, int a_global_strategy, AI policy, EvaluationFunction a_ef, boolean fensa) {
        super(available_time, max_playouts);
        MAXSIMULATIONTIME = lookahead;
        playoutPolicy = policy;
        MAX_TREE_DEPTH = max_depth;
    ...
    public GuidedA3NW(int available_time, int max_playouts, int lookahead, int max_depth, float e_l, float e_g, float e_0, int a_global_strategy, AI policy, EvaluationFunction a_ef, boolean fensa, String classGeneratorAction, UnitTypeTable utt) {
        super(available_time, max_playouts);
        MAXSIMULATIONTIME = lookahead;
        playoutPolicy = policy;
        MAX_TREE_DEPTH = max_depth;
    ...
    public GuidedA3NW(int available_time, int max_playouts, int lookahead, int max_depth, float e_l,
    ...
    public GuidedA3NW(int available_time, int max_playouts, int lookahead, int max_depth,
            ...
            float e_0, float discout_0,
            AI policy, EvaluationFunction a_ef,
            boolean fensa, UnitTypeTable utt, String behavior, int qtdUnits, List<AI> abstraction, String name) {
        super(available_time, max_playouts);
        MAXSIMULATIONTIME = lookahead;
        playoutPolicy = policy;
        MAX_TREE_DEPTH = max_depth;
    ...
    @Override
    public AI clone() {
        return new GuidedA3NW(TIME_BUDGET, ITERATIONS_BUDGET, MAXSIMULATIONTIME, MAX_TREE_DEPTH, 
    ...
    public AI clone_for_Thread() {
        return new GuidedA3NW(TIME_BUDGET, ITERATIONS_BUDGET, MAXSIMULATIONTIME, MAX_TREE_DEPTH, 
                epsilon_l, discount_l, epsilon_g, discount_g, epsilon_0, discount_0, playoutPolicy.clone_for_Thread(), ef, 
                forceExplorationOfNonSampledActions);
    }
...
//            }            
    ...
    public void startNewComputation(int a_player, GameState gs) throws Exception {
        player = a_player;
        current_iteration = 0;
        tree = new GuidedA3NWithinMCTSNode(player, 1 - player, gs, null, ef.upperBound(gs), current_iteration++, forceExplorationOfNonSampledActions, utt, 
                behavior, qtdUnits, scripts);
...
//        System.out.println("HL: " + count + " time: " + (System.currentTimeMillis() - start) + " (" + available_time + "," + max_playouts + ")");
    ...
    public boolean iteration(int player) throws Exception {
        ...
        GuidedA3NWithinMCTSNode leaf = tree.selectLeaf(player, 1 - player, epsilon_l, epsilon_g, epsilon_0, global_strategy, MAX_TREE_DEPTH, current_iteration++);
...
//            System.out.println(total_runs + " - " + epsilon_0 + ", " + epsilon_l + ", " + epsilon_g);
    ...
    public PlayerAction getBestActionSoFar() {
        ...
        if (DEBUG >= 1) {
            GuidedA3NWithinMCTSNode best = (GuidedA3NWithinMCTSNode) tree.children.get(idx);
...
//            }
        ...
        for (int i = 0; i < tree.children.size(); i++) {
            GuidedA3NWithinMCTSNode child = (GuidedA3NWithinMCTSNode) tree.children.get(i);
            if (DEBUG >= 2) {
...
//            if (best == null || (child.accum_evaluation/child.visit_count)>(best.accum_evaluation/best.visit_count)) {
    ...
    public GuidedA3NWithinMCTSNode getTree() {
        return tree;
    }
