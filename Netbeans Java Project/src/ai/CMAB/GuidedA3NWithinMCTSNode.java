 ...
 * @author santi
 */
public class GuidedA3NWithinMCTSNode extends MCTSNode {

    //strategies epsilon_g
    public static final int E_GREEDY = 0;
    public static final int UCB1 = 1;
    public static final int HC = 2; //Hill Climbing = PGS
    public static final int HC_ST = 3; // Hill Climbing + Stratify = SSS
    public static final int CLUSTER = 4; //Apply Cluster + Sampling
    public static final int GA = 5; // POE
    ...
    HashMap<BigInteger, GuidedA3NWithinMCTSNode> childrenMap = new LinkedHashMap<>();    // associates action codes with children
    ...
    public GuidedA3NWithinMCTSNode(int maxplayer, int minplayer, GameState a_gs, GuidedA3NWithinMCTSNode a_parent,
            double a_evaluation_bound, int a_creation_ID, boolean fensa, UnitTypeTable a_utt,
            String behavior, int qtdUnits, List<AI> abstraction) throws Exception {
        this.utt = a_utt;
        parent = a_parent;
        gs = a_gs;
        ...
        if (gs.winner() != -1 || gs.gameover()) {
            ...
            type = 0;
            moveGenerator = new A3NWithinGenerator(gs, maxplayer, utt, behavior, qtdUnits, abstraction);
            ...
            type = 1;
            moveGenerator = new A3NWithinGenerator(gs, minplayer, utt, behavior, qtdUnits, abstraction);
            actions = new ArrayList<>();
            children = new ArrayList<>();
            unitActionTable = new LinkedList<>();
            multipliers = new BigInteger[moveGenerator.getChoices().size()];
    ...
    // Alternated Sampling:
    public GuidedA3NWithinMCTSNode selectLeaf(int maxplayer, int minplayer, float epsilon_l, float epsilon_g, float epsilon_0, int global_strategy, int max_depth, int a_creation_ID) throws Exception {
        ...
        if (children.size() > 0 && r.nextFloat() >= epsilon_0) {
            ...
            GuidedA3NWithinMCTSNode selected = null;
    ...
    public GuidedA3NWithinMCTSNode selectFromAlreadySampledEpsilonGreedy(float epsilon_g) throws Exception {
        if (r.nextFloat() >= epsilon_g) {
            GuidedA3NWithinMCTSNode best = null;
            for (MCTSNode pate : children) {
                if (type == 0) {
                    // max node:
                    if (best == null || (pate.accum_evaluation / pate.visit_count) > (best.accum_evaluation / best.visit_count)) {
                        best = (GuidedA3NWithinMCTSNode) pate;
                    }
                } else {
                    // min node:
                    if (best == null || (pate.accum_evaluation / pate.visit_count) < (best.accum_evaluation / best.visit_count)) {
            ...
            GuidedA3NWithinMCTSNode best = (GuidedA3NWithinMCTSNode) children.get(r.nextInt(children.size()));
            return best;
        }
    }

    public GuidedA3NWithinMCTSNode selectFromAlreadySampledUCB1(float C) throws Exception {
        GuidedA3NWithinMCTSNode best = null;
        double bestScore = 0;
        for (MCTSNode pate : children) {
    ...
    public GuidedA3NWithinMCTSNode selectLeafUsingLocalMABs(int maxplayer, int minplayer, float epsilon_l, float epsilon_g, float epsilon_0, int global_strategy, int max_depth, int a_creation_ID) throws Exception {
        PlayerAction pa2;
        BigInteger actionCode;

        if (children.isEmpty() || r.nextFloat() < 0.75f) {
            int index = 0;
            PlayerAction guidance;
            AI scGruide = (AI) abstraction.toArray()[index];
            //scGruide.reset();
            if (maxplayer == 0) {
                guidance = scGruide.getAction(type, gs);
            } else {
                guidance = scGruide.getAction(1 - type, gs);
            }
            ...
            GuidedA3NWithinMCTSNode pate = childrenMap.get(actionCode);
            if (pate == null) {
                actions.add(pa2);
                GameState gs2 = gs.cloneIssue(pa2);
                GuidedA3NWithinMCTSNode node = new GuidedA3NWithinMCTSNode(maxplayer, minplayer, gs2.clone(), this, evaluation_bound, a_creation_ID,
                        forceExplorationOfNonSampledActions, utt, behavior, qtdUnits, abstraction);
    ...
    public void propagateEvaluation(double evaluation, GuidedA3NWithinMCTSNode child) {
...
//        if (child!=null) System.out.println(evaluation);
        ...
        if (parent != null) {
            ((GuidedA3NWithinMCTSNode) parent).propagateEvaluation(evaluation, this);
        }
    }
    ...
    private GuidedA3NWithinMCTSNode selectFromAlreadySampledAlternate() {
