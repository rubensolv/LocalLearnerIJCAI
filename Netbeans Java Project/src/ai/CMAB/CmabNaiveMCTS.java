...
public class CmabNaiveMCTS extends AIWithComputationBudget implements InterruptibleAI {
    ...
    public CmabNaiveMCTS(int available_time, int max_playouts, int lookahead, int max_depth, float e_l, float e_g, float e_0, 
                        int a_global_strategy, AI policy, EvaluationFunction a_ef, boolean fensa, String classGeneratorAction, 
                        UnitTypeTable utt, List<AI> abstraction, String name) {
        super(available_time, max_playouts);
        MAXSIMULATIONTIME = lookahead;
        playoutPolicy = policy;
        MAX_TREE_DEPTH = max_depth;
    ...
    public AI clone_for_Thread() {
        return new CmabNaiveMCTS(TIME_BUDGET, ITERATIONS_BUDGET, MAXSIMULATIONTIME, MAX_TREE_DEPTH, epsilon_l, discount_l, epsilon_g, discount_g, epsilon_0, discount_0, playoutPolicy.clone_for_Thread(), ef, forceExplorationOfNonSampledActions);
    }
    ...
    public void startNewComputation(int a_player, GameState gs) throws Exception {
        player = a_player;
        current_iteration = 0;
        tree = new CmabNaiveMCTSNode(player, 1 - player, gs, null, ef.upperBound(gs), current_iteration++, 
                                    forceExplorationOfNonSampledActions, utt, classGeneratorMove, scripts);
