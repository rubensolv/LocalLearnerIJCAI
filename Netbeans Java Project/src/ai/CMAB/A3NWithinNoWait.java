 ...
 *
 */
public class A3NWithinNoWait extends AIWithComputationBudget implements InterruptibleAI {
    ...
    protected A3NWithinMCTSNodeNoWait tree = null;
    ...
    public A3NWithinNoWait(UnitTypeTable utt) {
        this(100, -1, 100, 10,
                0.3f, 0.0f, 0.4f,
                new RandomBiasedAI(),
    ...
    public A3NWithinNoWait(int available_time, int max_playouts, int lookahead, int max_depth,
    ...
    public A3NWithinNoWait(int available_time, int max_playouts, int lookahead, int max_depth, float e_l, float e_g, float e_0, AI policy, EvaluationFunction a_ef, boolean fensa) {
        super(available_time, max_playouts);
        MAXSIMULATIONTIME = lookahead;
        playoutPolicy = policy;
        MAX_TREE_DEPTH = max_depth;
    ...
    public A3NWithinNoWait(int available_time, int max_playouts, int lookahead, int max_depth, float e_l, float e_g, float e_0, AI policy, EvaluationFunction a_ef, boolean fensa, String classGeneratorAction) {
        super(available_time, max_playouts);
        MAXSIMULATIONTIME = lookahead;
        playoutPolicy = policy;
        MAX_TREE_DEPTH = max_depth;
    ...
    public A3NWithinNoWait(int available_time, int max_playouts, int lookahead, int max_depth, float e_l, float e_g, float e_0, int a_global_strategy, AI policy, EvaluationFunction a_ef, boolean fensa) {
        super(available_time, max_playouts);
        MAXSIMULATIONTIME = lookahead;
        playoutPolicy = policy;
        MAX_TREE_DEPTH = max_depth;
    ...
    public A3NWithinNoWait(int available_time, int max_playouts, int lookahead, int max_depth, float e_l, float e_g, float e_0, int a_global_strategy, AI policy, EvaluationFunction a_ef, boolean fensa, String classGeneratorAction, UnitTypeTable utt) {
        super(available_time, max_playouts);
        MAXSIMULATIONTIME = lookahead;
        playoutPolicy = policy;
        MAX_TREE_DEPTH = max_depth;
    ...
    public A3NWithinNoWait(int available_time, int max_playouts, int lookahead, int max_depth, float e_l,
    ...
    @Override
    public AI clone() {
        return new A3NWithinNoWait(TIME_BUDGET, ITERATIONS_BUDGET, MAXSIMULATIONTIME, MAX_TREE_DEPTH, epsilon_l, discount_l, epsilon_g, discount_g, epsilon_0, discount_0, playoutPolicy, ef, forceExplorationOfNonSampledActions);
    }
    
    @Override
    public AI clone_for_Thread() {
        return new A3NWithinNoWait(TIME_BUDGET, ITERATIONS_BUDGET, MAXSIMULATIONTIME, MAX_TREE_DEPTH, epsilon_l, discount_l, epsilon_g, discount_g, epsilon_0, discount_0, 
                playoutPolicy.clone_for_Thread(), ef, forceExplorationOfNonSampledActions);
    }
    ...
    public void startNewComputation(int a_player, GameState gs) throws Exception {
        player = a_player;
        current_iteration = 0;
        tree = new A3NWithinMCTSNodeNoWait(player, 1 - player, gs, null, ef.upperBound(gs), current_iteration++, forceExplorationOfNonSampledActions, utt, behavior, qtdUnits, scripts);
    ...
    public void computeDuringOneGameFrame() throws Exception {
        if (DEBUG >= 2) {
            ...
            System.out.println("Game state " + this.gs_to_start_from.getTime());
        }
...
//        System.out.println("HL: " + count + " time: " + (System.currentTimeMillis() - start) + " (" + available_time + "," + max_playouts + ")");
    ...
    public boolean iteration(int player) throws Exception {
        ...
        A3NWithinMCTSNodeNoWait leaf = tree.selectLeaf(player, 1 - player, epsilon_l, epsilon_g, epsilon_0, global_strategy, MAX_TREE_DEPTH, current_iteration++);
...
//            System.out.println(total_runs + " - " + epsilon_0 + ", " + epsilon_l + ", " + epsilon_g);
    ...
    public PlayerAction getBestActionSoFar() {
        ...
        if (DEBUG >= 1) {
            A3NWithinMCTSNodeNoWait best = (A3NWithinMCTSNodeNoWait) tree.children.get(idx);
    ...
    public int getMostVisitedActionIdx() {
        ...
        int bestIdx = -1;
        A3NWithinMCTSNodeNoWait best = null;
        if (DEBUG >= 2) {
...
//            }
        ...
        for (int i = 0; i < tree.children.size(); i++) {
            A3NWithinMCTSNodeNoWait child = (A3NWithinMCTSNodeNoWait) tree.children.get(i);
            if (DEBUG >= 2) {
...
//            if (best == null || (child.accum_evaluation/child.visit_count)>(best.accum_evaluation/best.visit_count)) {
    ...
    public A3NWithinMCTSNodeNoWait getTree() {
        return tree;
    }
