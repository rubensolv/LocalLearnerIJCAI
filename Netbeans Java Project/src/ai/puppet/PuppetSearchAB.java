...
public class PuppetSearchAB extends PuppetBase {
    ...
    public PlayerAction getAction(int player, GameState gs) throws Exception {
        ...
        if (DEBUG >= 2) {
            System.out.println("Starting ABCD at frame " + gs.getTime() + ", player " + player + " with " + TIME_BUDGET + " ms");
        }
        if (!stack.empty()) {
        ...
        while (!stack.isEmpty() && !frameBudgetExpired() && !searchDone()) {
            ...
            if (current.prevMove == null) {//first side to choose move
                if (current.depth == maxDepth || current.gs.gs.gameover()) {//evaluate
                    ...
                    if (tt) {
                        TT.store(current.gs, current.best.m, current.best.score, current.alpha, current.beta, maxDepth - current.depth);
                    }
                ...
                if (current.nextMoves.hasNext()) {//check children
                    ...
                    if (tt) {
                        TT.store(current.gs, current.depth, current.prevMove, current.best.m, current.best.score, current.alpha, current.beta, maxDepth - current.depth);
                    }
                }
            }
